...
| `RESEND_FROM_EMAIL` | No | Verified sending address in Resend (e.g. `Just Genius <hello@justgeniusrecruiting.com>`). Falls back to Resend's shared test sender, which works for development but should be replaced with a verified domain before launch. |
